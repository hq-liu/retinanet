...
class RetinaNet_Shuffle(nn.Module):
    num_anchors = 9

    def __init__(self, num_classes=20):
        super(RetinaNet_Shuffle, self).__init__()
        self.fpn = FPN_ShuffleNet()
    ...
    def forward(self, x):
        ...
        for fm in fms:
            ...
            loc_pred = loc_pred.permute(0, 2, 3, 1).\
                contiguous().view(x.size(0), -1, 4)  # [N, 9*4,H,W] -> [N,H,W, 9*4] -> [N,H*W*9, 4]
            cls_pred = cls_pred.permute(0, 2, 3, 1).\
                contiguous().view(x.size(0), -1, self.num_classes+1)  # [N,9*20,H,W] -> [N,H,W,9*20] -> [N,H*W*9,20]
