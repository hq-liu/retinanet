...
def resize(img, boxes, size, max_size=1000):
    ...
    if isinstance(size, int):
        ...
        sw = float(ow) / w
        sh = float(oh) / h
    return img.resize((ow, oh), Image.BILINEAR), boxes*torch.FloatTensor([sw, sh, sw, sh])
...
def center_crop(img, boxes, size):
    ...
    boxes -= torch.FloatTensor([j, i, j, i])
...
def test():
    img = Image.open('D:\\VOCdevkit\\VOC2007\\JPEGImages\\000005.jpg')
    boxes = torch.FloatTensor([[48, 240, 195, 371], [8, 12, 352, 498]])
